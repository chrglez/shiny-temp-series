...
## Caso de que necesite limpiar la serie de outliers
...
# them with linearly interpolated replacements.
...
# En lo que respecta al script he utilizaso TNSO.ts (la serie sin outliers)
# OJO
...
# Perform STL decomposition (por defecto esquema multiplicativo)

stl_decomposed_data <- stl(TNSO.ts, s.window = "periodic")

# Plot the decomposition
plot(stl_decomposed_data)

trend.stl <- stl_decomposed_data$time.series[, "trend"]
seasonal.stl <- stl_decomposed_data$time.series[, "seasonal"]
residuals.stl <- stl_decomposed_data$time.series[, "remainder"]

plot(trend.stl)

# Multiplicative version using stl trend

ywt.mstl <- TNSO.ts/trend.stl

season.Matrix = matrix(data = ywt.mstl,
                       byrow = FALSE, nrow = 12)

seasonal.mstl <- rowMeans(season.Matrix) / mean(rowMeans(season.Matrix)) 

mean(seasonal.mstl)

residual.mstl <- TNSO.ts/((trend.stl)*(rep(seasonal.mstl, 20)))

plot(residual.mstl) # multiplicative residual plot


# Aditive version using stl trend

ywt.astl <- TNSO.ts-trend.stl

season.Matrix = matrix(data = ywt.astl,
                       byrow = FALSE, nrow = 12)

seasonal.astl <- rowMeans(season.Matrix) - mean(rowMeans(season.Matrix)) 

mean(seasonal.astl)

residual.astl <- TNSO.ts-(trend.stl)-rep(seasonal.astl, 20)

plot(residual.astl) # additive residual plot
...
# Convertir a matriz año × periodo
...
#######################################################################################
# Segundo módulo: pruebas estadísticas de estacionalidad mediante modelización arima #
#####################################################################################

#####################################################
# Identificación automática de la estructura arima #
###################################################
...
Kuiper2sample(Vari.estacional, DH, tail = TRUE, conservative = FALSE)## Contraste de kolmogorov-Smirnov
# The KS test compares two ECDFs by looking at the maximum difference between them
# Hipótesis Nula: La distribución de la muestra es consistente con la distribución teórica.
ks.test(Vari.estacional/12, DH, alternative = "two.sided")
...
# Utilizar esta otra versión del test de Kuiper sin simulación

Kuiper2sample(Vari.estacional, DH, tail = TRUE, conservative = FALSE)

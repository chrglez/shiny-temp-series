...
#' Ejecutar suite de tests de estacionalidad
#' @param ts_data Serie temporal
#' @return data.frame con resultados de los tests
run_seasonality_tests <- function(ts_data) {
  results <- data.frame(
    Test = character(),
    Statistic = numeric(),
    PValue = numeric(),
  ...
  # Test combinado (Ollech-Webel)
  tryCatch({
    combined <- seastests::combined_test(ts_data)
    results <- rbind(results, data.frame(
      Test = "Combined (Ollech-Webel)",
      Statistic = NA,
      PValue = combined$Pval,
      Significant = ifelse(combined$Pval < 0.05, "Sí", "No")
    ))
  }, error = function(e) NULL)

  # QS Test
  tryCatch({
    qs <- seastests::qs(ts_data)
    results <- rbind(results, data.frame(
      Test = "QS Test",
      Statistic = qs$stat,
      PValue = qs$Pval,
      Significant = ifelse(qs$Pval < 0.05, "Sí", "No")
    ))
  }, error = function(e) NULL)

  # Kruskal-Wallis
  tryCatch({
    kw <- seastests::kw(ts_data)
    results <- rbind(results, data.frame(
      Test = "Kruskal-Wallis",
      Statistic = kw$stat,
      PValue = kw$Pval,
      Significant = ifelse(kw$Pval < 0.05, "Sí", "No")
    ))
  }, error = function(e) NULL)

  # Friedman
  tryCatch({
    fried <- seastests::fried(ts_data)
    results <- rbind(results, data.frame(
      Test = "Friedman",
      Statistic = fried$stat,
      PValue = fried$Pval,
      Significant = ifelse(fried$Pval < 0.05, "Sí", "No")
    ))
  }, error = function(e) NULL)
...
#' @param x Vector numérico 1
#' @param y Vector numérico 2
#' @return data.frame con resultados de comparación
...
#' @return Lista con outliers detectados y serie limpia
detect_outliers <- function(ts_data, method = "iqr") {
  if (method == "iqr") {
    ...
    lower <- q1 - 1.5 * iqr
    upper <- q3 + 1.5 * iqr
    outliers <- which(ts_data < lower | ts_data > upper)
  } else {
    outliers <- integer()
  }
